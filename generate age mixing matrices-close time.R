...
random_contact_id <- function(id) {
  contact<-sample(contact_data[which(contact_data$id==id),5],1)
  return(contact)
}
...
get_contact_minutes <-function(contactID) {
  contact_minutes<-contact_data[which(contact_data$contact_id==contactID),4]
  return(contact_minutes)
}
...
for (agecat in seq(1,agecat_number)) {
  ...
  for (contact_agecat in seq(0,agecat_number)) {
    ...
    best_contact_rates[agecat,contact_agecat+1]<- sum(a$contact_weight*a$contactMinutes /a$weight)/respondent_number_subset_weighted
    a<-respondent_data_subset[which(respondent_data_subset$contactage==contact_agecat & respondent_data_subset$hhMem==1),]
    best_contact_rates_home[agecat,contact_agecat+1]<- sum(a$contact_weight*a$contactMinutes /a$weight)/respondent_number_subset_weighted
    a<-respondent_data_subset[which(respondent_data_subset$contactage==contact_agecat & respondent_data_subset$hhMem==0),]
    best_contact_rates_other[agecat,contact_agecat+1]<- sum(a$contact_weight*a$contactMinutes /a$weight)/respondent_number_subset_weighted
  }
}
...
for (agecat in seq(1,agecat_number)) {
  ...
  for (b in seq(1,bootstrap_number)) {
    ...
    respondent_data_sample$Hours<-as.numeric(unlist(lapply(respondent_data_sample$contactID,get_contact_minutes)))
    
    for (contact_agecat in seq(0,agecat_number)) {
      a<-respondent_data_sample[which(respondent_data_sample$contact_age==contact_agecat),]
      bootstrapped_contact_rates[agecat,contact_agecat+1,b]<- sum(a$Hours /a$weight)/respondent_number_subset_weighted
      a<-respondent_data_sample[which(respondent_data_sample$contact_age==contact_agecat & respondent_data_sample$hhMem==1),]
      bootstrapped_contact_rates_home[agecat,contact_agecat+1,b]<- sum(a$Hours /a$weight)/respondent_number_subset_weighted
      a<-respondent_data_sample[which(respondent_data_sample$contact_age==contact_agecat & respondent_data_sample$hhMem==0),]
      bootstrapped_contact_rates_other[agecat,contact_agecat+1,b]<- sum(a$Hours /a$weight)/respondent_number_subset_weighted
    }
